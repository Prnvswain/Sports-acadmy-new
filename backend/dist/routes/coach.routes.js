...
router.patch('/:id', (0, auth_1.requireRoles)(client_1.UserRole.ACADEMY_ADMIN), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    const { academyId } = req;
    const coach = await prisma_1.prisma.coach.findUnique({ where: { id: req.params.id } });
    if (!coach)
        throw new errors_1.NotFoundError();
    (0, tenantQuery_1.assertTenantMatch)(coach.academyId, academyId);
    ...
    const updated = await prisma_1.prisma.coach.update({ where: { id: req.params.id }, data: body });
