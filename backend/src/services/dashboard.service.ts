...
export async function getAdminDashboard(academyId: string) {
  const now = new Date();
  ...
  const [
    ...
    prisma.student.count({ where: { academyId, isActive: true, deletedAt: null } }),
    prisma.coach.count({ where: { academyId, isActive: true } }),
    prisma.batch.count({ where: { academyId, isActive: true } }),
    ...
    getSubscriptionStatus(academyId),
  ]);
...
export async function getCoachDashboard(academyId: string, coachId: string) {
  const today = new Date().toISOString().slice(0, 10);
  const monthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1);

  const batches = await prisma.batchCoach.findMany({
    where: { coachId },
    include: {
      batch: {
        include: {
  ...
  const ownAttendance = await prisma.coachAttendance.findFirst({
    where: { coachId, date: new Date(today) },
  });

  const monthlyAttendance = await prisma.coachAttendance.groupBy({
    by: ['status'],
    where: { coachId, date: { gte: monthStart } },
    _count: true,
  });

  const pendingPerformance = await prisma.student.count({
    where: {
      academyId,
  ...
  const todayAttendanceMarked = await prisma.studentAttendance.count({
...
export async function getSuperAdminDashboard() {
  const [academies, revenue, planBreakdown] = await Promise.all([
    prisma.academy.groupBy({ by: ['status'], _count: true }),
    prisma.feePayment.aggregate({ where: { status: 'PAID' }, _sum: { amount: true } }),
    prisma.academy.groupBy({ by: ['subscriptionPlan'], _count: true }),
  ]);
...
async function getRevenueTrend(academyId: string) {
...
async function getStudentGrowth(academyId: string) {
  const students = await prisma.student.findMany({
    where: withTenant(academyId, { deletedAt: null }),
...
async function getAttendanceTrend(academyId: string) {
...
async function getSportsDistribution(academyId: string) {
