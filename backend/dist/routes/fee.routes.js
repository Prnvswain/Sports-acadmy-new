...
router.get('/dashboard', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [totalRevenue, monthlyRevenue, pending, overdue] = await Promise.all([
        prisma_1.prisma.feePayment.aggregate({
            where: (0, tenantQuery_1.withTenant)(academyId, { status: 'PAID' }),
            _sum: { amount: true },
        }),
        prisma_1.prisma.feePayment.aggregate({
            where: (0, tenantQuery_1.withTenant)(academyId, { status: 'PAID', paidDate: { gte: monthStart } }),
            _sum: { amount: true },
        }),
        prisma_1.prisma.feePayment.aggregate({
            where: (0, tenantQuery_1.withTenant)(academyId, { status: 'PENDING' }),
            _sum: { amount: true },
        }),
        prisma_1.prisma.feePayment.aggregate({
            where: (0, tenantQuery_1.withTenant)(academyId, { status: 'OVERDUE' }),
            _sum: { amount: true },
        }),
    ]);
...
router.post('/:id/collect', (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const payment = await prisma_1.prisma.feePayment.findUnique({ where: { id: req.params.id } });
    if (!payment)
